# =============================================================================
# Boxplot showing nodule size distribution: Gs11_36243818:C-T
# =============================================================================
...
# ---------------------
geno <- read.table("genotipo_Gs11_36243818.txt", header = TRUE, sep = "\t",
...
# Remove accesiones with 0 nodules (132, 134, 212) and the single heterozygous
# sample (190) as it provides insufficient statistical power for genotype comparison
remove_samples <- c("132", "134", "212", "190")
...
# Compute percentage difference in mean weight between genotypes
(mean_by_genotype$mean_weight[2] - mean_by_genotype$mean_weight[1]) / 
  mean_by_genotype$mean_weight[1] * 100

# Split data by genotype
group_CC <- df_plot$Weight_per_nodule_mean[df_plot$genotipo == "CC"]
group_TT <- df_plot$Weight_per_nodule_mean[df_plot$genotipo == "TT"]

# Test normality for each genotype group
shapiro.test(group_CC)
shapiro.test(group_TT)

# Compare genotype groups using Wilcoxon rank-sum test
wilcox_test <- wilcox.test(group_CC, group_TT)
print(wilcox_test)
...
p <- ggplot(df_plot, aes(x = genotipo, y = Weight_per_nodule_mean)) +
  ...
  geom_jitter(shape = 1, color = "#8B0000", size = 2.5, stroke = 0.7, 
              width = 0.15, height = 0) +
  ggsignif::geom_signif(comparisons = list(c("CC", "TT")), 
                        annotations = "***",
                        y_position = max(df_plot$Weight_per_nodule_mean, na.rm = TRUE) * 1.05,
                        tip_length = 0.03) +
  labs(x = "Gs11:36243818:C-T", y = "Nodule size (g)") +
